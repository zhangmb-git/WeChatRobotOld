...
// 返 回 值: void
//************************************************************
void SetWxRoomAnnouncement(wchar_t* chatroomwxid, const wchar_t* Announcement) {
    //获取WeChatWin的基址
...
void ShowChatRoomUser(wchar_t* chatroomwxid) {
    ...
    userinfodata.cbData = ((wcslen(chatroomwxid) + 1) * 2);     //strlen(szSendBuf);//待发送的数据的长
    userinfodata.lpData = chatroomwxid;  // szSendBuf;//待发送的数据的起始地址(可以为NULL)
    SendMessage(hWnd, WM_COPYDATA, (WPARAM)hWnd, (LPARAM)&userinfodata);

    //完成之后发送消息到主界面
    HWND hMainWnd = FindWindow(NULL, TEXT("微信助手"));
    ...
    if (hMainWnd != NULL) {
        ...
        userinfodata.cbData = ((wcslen(chatroomwxid) + 1) * 2);// strlen(szSendBuf);//待发送的数据的长
        userinfodata.lpData = chatroomwxid;// szSendBuf;//待发送的数据的起始地址(可以为NULL)
        SendMessage(hMainWnd, WM_COPYDATA, (WPARAM)hMainWnd, (LPARAM)&userinfodata);
        return;
    }
...
void ShowChatRoomMember(wchar_t* chatroomwxid, wchar_t* userwxid) {
    struct wechatText {
        wchar_t* pStr;
        int strLen;
        int iStrLen;
    ...
    swprintf_s(userinfo->GroupID, L"%s", chatroomwxid);  //添加群组ID
    ...
    if (hMainWnd != NULL) {
        COPYDATASTRUCT userinfodata;
        userinfodata.dwData = WM_ShowChatRoomMember;//保存一个数值, 可以用来作标志等
        userinfodata.cbData = sizeof(UserInfo);// strlen(szSendBuf);//待发送的数据的长
        userinfodata.lpData = userinfo;// szSendBuf;//待发送的数据的起始地址(可以为NULL)
        SendMessage(hMainWnd, WM_COPYDATA, (WPARAM)hMainWnd, (LPARAM)&userinfodata);
        //return;
...
void GetUserInfoByWxid(wchar_t* userwxid) {
    ...
    swprintf_s(userinfo->UserNickName, L"%s", (wchar_t*)(*((LPVOID*)wxNickAdd)));
    swprintf_s(userinfo->UserNumber, L"%s", (wchar_t*)(*((LPVOID*)wxuserIDAdd)));
    //swprintf_s(userinfo->GroupID, L"%s", userwxid);  //
...
// 返 回 值: void
//************************************************************
void SendRoomAtMsg(std::wstring chatroomid, std::wstring memberwxid, std::wstring membernickname, std::wstring content) {
    ...
    CString  szTempMsg;
    szTempMsg.Format(_T("@%s %s"), membernickname.c_str(), content.c_str());
    std::wstring tempmsg = szTempMsg.GetString();

    //swprintf(&tempmsg[0],  L"@%s %s", membernickname.c_str(), content.c_str());

    TEXT_WX wxMsg(tempmsg);

    TEXT_WXID wxAtId;
    wxAtId.pWxid = memberwxid;
    wxAtId.length = wcslen(memberwxid.c_str());
    wxAtId.maxLength = wcslen(memberwxid.c_str()) * 2;
    wxAtId.fill1 = 0;
    wxAtId.fill2 = 0;
